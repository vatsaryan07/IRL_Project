...
class IRLEnv(FrozenLakeEnv):
    ...
    def __init__(
        ...
        self.mP = {s: {a: [] for a in range(self.nA)} for s in range(self.nS)}
        def to_s(row, col):
            return row * ncol + col
            ...
            for col in range(ncol):
                ...
                for a in range(4):
                    ...
                    if letter in b"H":
                        ...
                        mli.append((1.0, s, -5.0, True))
                    elif letter in b"G":
                        li.append((1.0, s, 50.0, True))
                        mli.append((1.0, s, -5.0, True))
                    else:
                        if self.is_slippery:
                            for b, val in zip([(a - 1) % 4, a, (a + 1) % 4], [0.05, 0.9, 0.05]):
                                li.append(
                                    (val, *update_probability_matrix(row, col, b))
                                )
                            mli.append((1.0, *update_probability_matrix(row, col, a)))
                        else:
    ...
    def step(self, action, monster_action=None):
        ...
        monster_transitions = self.mP[self.m][monster_action]
