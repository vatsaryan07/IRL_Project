...
if __name__=="__main__":
    ...
    n_total_feats = len(all_agent_locs)  + n_additional_feats
    feat_map = np.zeros((len(all_agent_locs), n_total_feats))
    ...
    with open("feat_map_fl.npy", "wb") as f:
        np.save(f, feat_map)
    ...
    trans_matrix = np.zeros((len(all_agent_locs),
                             len(all_agent_locs),
                             actions.size))

    for a_idx, a in enumerate(all_agent_locs):
        for action in actions:
            curr_state_idx = a_idx 
            next_agent_state = agent_next_states[a_idx][action]
            next_state_idx = np.array([next_agent_state ])
            trans_matrix[curr_state_idx, next_state_idx, action] = 1.0 / next_state_idx.size

    with open("trans_matrix_fl.npy", "wb") as f:
