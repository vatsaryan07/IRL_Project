...
#         return np.array([s]), r, t, False, {"prob": p}
...
class IRLEnv(FrozenLakeEnv):
            ...
            for col in range(ncol):
                ...
                for a in range(4):
                    ...
                    if letter in b"H":
                        ...
                        if self.is_slippery:
                            for b, val in zip([(a - 1) % 4, a, (a + 1) % 4], [0.1, 0.8, 0.1]):
                                li.append(
                                    (val, *update_probability_matrix(row, col, b))
                                )
                        else:
                            li.append((1.0, *update_probability_matrix(row, col, a)))

        # self.observation_space = spaces.Discrete(nS)
        dummy_obs = self.get_obs_vector(0)
    ...
    def step(self, action, monster_action=None):
        ...
        # if s in hole_locations:
        #     # if eaten by monster
        #     r = -5.0  # Set negative reward
            # t = True  # End episode
        
        if self.render_mode == "human":
            self.render()
        # truncation=False as the time limit is handled by the `TimeLimit` wrapper added during `make`
        return self.get_obs_vector(s), r, t, False, {"prob": p}
    ...
    def reset(
        ...
        if self.render_mode == "human":
            self.render()
        return self.get_obs_vector(self.s), {"prob": 1}


if __name__ == '__main__':
    env = IRLEnv(render_mode="human", version=5)
    obs, info = env.reset()
    ACTIONS = {"a": 0,
               "s": 1,
               "d": 2,
